...
def create_1m_rtn(
    ...
    # 기준년울 컬럼을 생성해서 index에서 년도-월 추출하여  대입
...
# 데이터를 로드하고 월별 수익률을 계산하여 데이터프레임에 추가하는 함수
def data_load(
        _path = "./data",
        _ext = 'csv', 
        _start = '2010-01-01',
...
def create_trading(_df, _codes): 
    ...
    for code in _codes: # 10번 반복
        # 인덱스를 기준으로 반복문 생성
        for idx in df.index:
            # 특정 종목의 포지션을 잡는다. (전 행의 p_code 컬럼이 ready)이고
            # (현재 행의 p_code 컬럼이 "")
...
def multi_return(_df, _codes):
    ...
    for idx in df.index:
        ...
        for code in _codes:
            ...
            if df.loc[idx, f"p_{code}"] == "":  #매수 매도 가격이 없어야 함.
                 buy_dict[code] = 0
                 sell_dict[code] = 0
    return df
